...
export default function Crismandos() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editId) {
        await updateDoc(doc(db, 'crismandos', editId), data);
      } else {
        await addDoc(collection(db, 'crismandos'), { ...data, dataCadastro: new Date().toISOString() });
      }
      setShowForm(false);
    } finally {
      setSaving(false);
  ...
  const handleDelete = async (id) => {
    await deleteDoc(doc(db, 'crismandos', id));
            ...
            <div key={c.id} className="card p-0 overflow-hidden">
              ...
              >
                <div className="w-9 h-9 rounded-full bg-navy-100 flex items-center justify-center shrink-0">
                  <span className="text-sm font-semibold text-navy-700">{c.nome[0].toUpperCase()}</span>
                </div>
              ...
              {isOpen && (
                <div className="border-t border-gray-100 px-4 py-4 bg-gray-50 space-y-4">
                  ...
                  <div className="flex flex-wrap gap-2 pt-1">
                    ...
                    {isCoordenacao && (
                      <>
                        <button onClick={() => openForm(c)} className="btn-secondary text-xs py-1.5 px-3">
                          <Edit2 size={13} /> Editar
                        </button>
                        <button onClick={() => setConfirmDelete(c.id)} className="btn-danger text-xs py-1.5 px-3">
            ...
            <div className="px-6 py-5 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input value={phoneMask(form.telefone)} onChange={e => setForm({...form, telefone: e.target.value.replace(/\D/g, '')})} className="input-field" placeholder="(11) 99999-9999" />
                </div>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">Nome dos Pais / Responsáveis</label>
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white rounded-xl shadow-xl p-6 max-w-sm w-full">
            ...
            <p className="text-gray-500 text-sm mb-5">Tem certeza que deseja excluir este crismando? Esta ação não pode ser desfeita.</p>
            <div className="flex gap-2 justify-end">
              <button onClick={() => setConfirmDelete(null)} className="btn-secondary">Cancelar</button>
              <button onClick={() => handleDelete(confirmDelete)} className="btn-danger">Excluir</button>
            </div>
          </div>
        </div>
      )}
